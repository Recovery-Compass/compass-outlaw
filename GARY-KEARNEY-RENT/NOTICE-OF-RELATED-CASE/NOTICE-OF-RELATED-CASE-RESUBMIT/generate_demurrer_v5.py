...
# --- Configuration ---
OUTPUT_FILENAME = "DEMURRER_FINAL_SUBMISSION_v5.pdf"
...
# MARGIN_TOP adjusted to 0.9 inch to raise grid
MARGIN_TOP = 0.9 * inch 
MARGIN_BOTTOM = 0.35 * inch # Relaxed to allow footer at 0.35
...
# New Grid Start: Raise text block by start Y
...
def create_demurrer_v5():
    ...
    # Signature Page 3
    signature_y = get_y(28)
    c.drawString(MARGIN_LEFT, signature_y, "_" * 35)
    
    # Text BELOW Line 28
    # With footer at 0.35 inch (~25 pts), and Line 28 baseline at ~1.5 inch (due to 24*28=672. 11in - 1in_top - 9.33 = 0.67in?)
    # Let's check math:
    # 28 lines * 24pts = 672 pts.
    # 11 inches = 792 pts.
    # Top Margin 0.9 inch = 64.8 pts.
    # Start Baseline (Line 1) = 792 - 64.8 - 18 = 709.2 pts.
    # Line 28 Baseline = 709.2 - (27 * 24) = 709.2 - 648 = 61.2 pts.
    # 61.2 pts = 0.85 inches from bottom.
    # Footer at 0.35 inch = 25.2 pts.
    # Gap = 61.2 - 25.2 = 36 pts (0.5 inch space).
    # Signature name at -14 pts = 61.2 - 14 = 47.2 pts.
    # Title at -28 pts = 61.2 - 28 = 33.2 pts.
    # Footer Top (font height ~10) -> 25.2 + 10 = 35.2 pts.
    # Title at 33.2 might just brush footer top.
    # Safer to keep Footer at 0.35 or 0.3. Let's stick to 0.35 (User commanded). 
    # Title at -28 might be close. Let's do -26.
    
    name_y = signature_y - 12
    title_y = signature_y - 24 # Tighter than -28 to be safe 
    
    c.setFont("Times-Bold", 12)
    c.drawString(MARGIN_LEFT, name_y, "NUHA SAYEGH")
    c.setFont("Times-Roman", 12)
    c.drawString(MARGIN_LEFT, title_y, "Defendant in Pro Per")
    
    c.save()
    print(f"Final Footer Fix PDF generated at: {FULL_OUTPUT_PATH}")
