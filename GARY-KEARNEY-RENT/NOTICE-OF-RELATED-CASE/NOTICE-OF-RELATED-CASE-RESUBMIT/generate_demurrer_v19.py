...
# --- Configuration ---
OUTPUT_FILENAME = "DEMURRER_FINAL_TITANIUM.pdf"
...
def create_demurrer_v19():
    ...
    # --- CAPTION BOX STRUCTURE (v19 TITANIUM LOCK) ---
    c.saveState()
    c.setLineWidth(0.5)
    ...
    # 2. Bottom Seal (Floor): Midway between Line 20 and 21 (Closing Box after text)
    ...
    # 3. Vertical Separator (Perfect T-Joint Connection)
    ...
    # Fix v19: SEP_X was defined in the block above, but local scope might be tricky if not careful. 
    # Actually, SEP_X was defined inside the `with c.saveState()` block or just before it? 
    # In my previous code: 
    # SEP_X = 4.5 * inch
    # ... c.restoreState()
    # So SEP_X is still in scope? Yes, Python function scope. 
    # BUT I named it SEP_X (uppercase). The line below uses sep_x (lowercase).
    SEP_X = 4.5 * inch
    info_x = SEP_X + 10
    c.setFont("Times-Bold", 12)
    ...
    # v17/19 ALIGN CAPTION FIELDS maintained
    c.setFont("Times-Roman", 12)
    ...
    c.save()
    print(f"Final Titanium PDF generated at: {FULL_OUTPUT_PATH}")
