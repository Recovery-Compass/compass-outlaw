...
# --- Configuration ---
OUTPUT_FILENAME = "DEMURRER_FINAL_SEALED.pdf"
...
def create_demurrer_v16():
    ...
    # --- CAPTION BOX STRUCTURE (v16 SEALED BOX) ---
    c.saveState()
    c.setLineWidth(0.5)
    
    # Coordinates
    top_seal_y = get_y(11) + 10 # Ceiling (Just above Line 11 text)
    seal_y = get_y(22) + 12     # Floor (Just above Line 22 text)
    sep_x = 4.5 * inch
    left_rail_inner_x = 1.35 * inch
    right_rail_x = PAGE_WIDTH - 0.5 * inch
    
    # 1. Top Horizontal Line (The Roof)
    c.line(left_rail_inner_x, top_seal_y, right_rail_x, top_seal_y)
    
    # 2. Bottom Horizontal Line (The Floor)
    c.line(left_rail_inner_x, seal_y, right_rail_x, seal_y)
    
    # 3. Vertical Separator (Extended to touch Roof and Floor)
    ...
    # ================= PAGE 3 - EMERALD+STRUCTURE+MODERN+SEALED EDITION =================
    ...
    c.save()
    print(f"Final Sealed PDF generated at: {FULL_OUTPUT_PATH}")
