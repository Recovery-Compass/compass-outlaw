...
# --- Configuration ---
OUTPUT_FILENAME = "DEMURRER_FINAL_PLATINUM.pdf"
...
def create_demurrer_v11():
    ...
    # Lines 8-11: COMPRESSED Intro Body (4 lines)
    c.setFont("Times-Roman", 12)
    c.drawString(MARGIN_LEFT, get_y(8), "This retaliatory Unlawful Detainer attempts to enforce a void lease on an illegal")
    
    # Line 9
    draw_manual_rich_line(c, MARGIN_LEFT, 9, [
        ("dwelling. Defendant previously filed ", "Times-Roman"),
        ("Sayegh v. Kearney", "Times-Italic"),
        (" (Case No. 26NNCV00412)", "Times-Roman")
    ])
    
    c.drawString(MARGIN_LEFT, get_y(10), "on January 21, 2026, alleging Fraud and seeking Rescission. Because the First")
    c.drawString(MARGIN_LEFT, get_y(11), "Action challenges the lease's validity and was filed first, this Court must sustain the")
    c.drawString(MARGIN_LEFT, get_y(11.2) - 12, "demurrer.") # Small manual adjust to fit last word cleanly if needed, or check wrap. 
    # Let's trust wrapping or direct placement.  
    # "this Court must sustain the demurrer." fits on line 11 easy? 
    # "Action challenges..." line is long.
    # Let's stick to the prompt's hardcoded text logic.
    # Prompt implies it wraps to 4 lines.
    # I will be specific. 
    # "This retaliatory Unlawful Detainer attempts to enforce a void lease on an illegal dwelling. Defendant previously filed *Sayegh v. Kearney* (Case No. 26NNCV00412) on January 21, 2026, alleging Fraud and seeking Rescission. Because the First Action challenges the lease's validity and was filed first, this Court must sustain the demurrer."
    # Line 8: "This retaliatory Unlawful Detainer attempts to enforce a void lease on an illegal"
    # Line 9: "dwelling. Defendant previously filed Sayegh v. Kearney (Case No. 26NNCV00412)"
    # Line 10: "on January 21, 2026, alleging Fraud and seeking Rescission. Because the First"
    # Line 11: "Action challenges the lease's validity and was filed first, this Court must sustain the demurrer."
    # The last line might be slightly long or short. I'll rely on the manual strings here to be safe and clear.
    
    # Let's overwrite line 11 cleanly
    c.drawString(MARGIN_LEFT, get_y(11), "Action challenges the lease's validity and was filed first, this Court must sustain the demurrer.")

    # Line 12: Blank (Spacer)
    
    # Line 13: Header II
    c.setFont("Times-Bold", 12)
    c.drawString(MARGIN_LEFT, get_y(13), "II. THE LEASE IS VOID AB INITIO (CCP § 430.10(e))")
    
    # Lines 14-18: Body II (5 lines)
    # Copied from previous, shifted to 14
    c.setFont("Times-Roman", 12)
    c.drawString(MARGIN_LEFT, get_y(14), "A contract for an illegal purpose is void (Civil Code § 1598). A lease for a unit that")
    ...
    c.drawString(MARGIN_LEFT, get_y(17), "and mandatory access width. A 3-Day Notice that demands rent for an illegal unit")
    c.drawString(MARGIN_LEFT, get_y(18), "is invalid on its face.")
    ...
    # Lines 21-23: Body III (3 lines)
    c.setFont("Times-Roman", 12)
    c.drawString(MARGIN_LEFT, get_y(21), "Under CCP § 430.10(c), a party may demur when \"there is another action pending")
    c.drawString(MARGIN_LEFT, get_y(22), "between the same parties on the same cause of action.\" The validity of the Lease")
    
    # Line 23
    draw_manual_rich_line(c, MARGIN_LEFT, 23, [
    ...
    # --- Bottom Execution Block (25-27/28) ---
    c.setFont("Times-Roman", 12)
    ...
    # Tighter Visuals
    c.setFont("Times-Bold", 12)
    c.drawString(MARGIN_LEFT, sig_y - 12, "NUHA SAYEGH") # Visually tight under line 26
    c.setFont("Times-Roman", 12)
    c.drawString(MARGIN_LEFT, sig_y - 24, "Defendant in Pro Per") # Visually tight under Name
    
    c.save()
    print(f"Final Platinum PDF generated at: {FULL_OUTPUT_PATH}")
