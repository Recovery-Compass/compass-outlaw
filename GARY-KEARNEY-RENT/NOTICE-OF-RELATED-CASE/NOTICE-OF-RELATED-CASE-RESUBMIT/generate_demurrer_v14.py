...
# --- Configuration ---
OUTPUT_FILENAME = "DEMURRER_FINAL_STRUCTURE.pdf"
...
def draw_grid_background(c, page_idx):
    ...
    # 3. Right Vertical Rail (Single Line) - NEW v14
...
def create_demurrer_v14():
    ...
    # --- CAPTION SEAL (v14 NEW) ---
    # Horizontal line exactly at top of Line 22 (separating caption from body)
    seal_y = get_y(22) + 12 # 12 pts above baseline of Line 22
    left_rail_x = 1.3 * inch
    right_rail_x = PAGE_WIDTH - 0.5 * inch
    
    c.saveState()
    c.setLineWidth(1.0) # Slightly thicker for seal? Or match rails. Prompt says "Solid Horizontal Line". Standard is same width as content or rails. Let's stick to 1.0 or 0.5. Let's do 1.0 specifically for the seal to make it visible. Actually standard rails are 0.5 so let's match rails usually. Prompt says "Solid Horizontal Line".  Wait, prompt doesn't specify weight. I'll use 0.5 to match rails.
    c.setLineWidth(0.5)
    c.line(left_rail_x, seal_y, right_rail_x, seal_y)
    c.restoreState()
    
    # Intro - ensuring text is safe below line
    c.setFont("Times-Bold", 12)
    # Line 22 baseline is `get_y(22)`. Line drawn at `get_y(22) + 12`.
    # Text ascenders might go up 10-12pts. It should be just fine. 
    # If explicitly requested, I could shift it down slightly, but Grid Lock is paramount.
    # The prompt says: "If necessary, push the text down 2-4 points".
    # I'll stick to the grid first. If it touches, user will feedback. 
    # Actually, in Pleading Paper, the text is ON the line. The separator is usually ABOVE the line structure or between 20 and 21 potentially?
    # Prompt says "This line sits exactly at Line 22". "Draw... at the top of Line 22... approx get_y(22) + 12".
    # The text "TO PLAINTIFF" is on Line 22. 
    # Let's trust the math: Baseline y. Line y+12. Font size 12. 
    # It will be tight but technically clear (separator is at mid-leading).
    ...
    # ================= PAGE 3 - EMERALD+STRUCTURE EDITION =================
    ...
    # Lines 8-11: Auto-Wrapped Intro Body
    ...
    # Line 27: Name (GRID SNAP - Exact Y)
    c.setFont("Times-Bold", 12)
    c.drawString(MARGIN_LEFT, get_y(27), "NUHA SAYEGH") 
    
    # Line 28: Title (GRID SNAP - Exact Y)
    c.setFont("Times-Roman", 12)
    c.drawString(MARGIN_LEFT, get_y(28), "Defendant in Pro Per") 
    
    c.save()
    print(f"Final Structure PDF generated at: {FULL_OUTPUT_PATH}")
