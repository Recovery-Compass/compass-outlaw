...
# --- Configuration ---
OUTPUT_FILENAME = "DEMURRER_FINAL_FILING_READY.pdf"
...
def create_demurrer_v20():
    ...
    # --- CAPTION BOX STRUCTURE (v20 DIAMOND-PLATE TITANIUM) ---
    c.saveState()
    c.setLineWidth(0.5)
    
    # TITANIUM COORDINATES (Exact Definitions)
    # 1. Top Seal (Roof): Just above Line 11 Text
    TOP_SEAL_Y = get_y(11) + 12 
    
    # 2. Bottom Seal (Floor): Midway between Line 20 and 21
    BOTTOM_SEAL_Y = get_y(21) + 12
    
    # 3. X-Coordinates
    SEP_X = 4.5 * inch
    LEFT_RAIL_X = 1.3 * inch + 2 # Snap to right edge of double rail
    RIGHT_RAIL_X = PAGE_WIDTH - 0.5 * inch
    
    # DRAW SEQUENCE (Using Exact Vars)
    ...
    # Fix v19 variable scope issue explicitly again
    SEP_X_VAR = 4.5 * inch
    info_x = SEP_X_VAR + 10
    ...
    # v20 VECTOR CAPTION FIELDS
    c.setFont("Times-Roman", 12)
    field_value_x = info_x + 0.6 * inch
    field_line_length = 1.5 * inch
    
    # Helper to draw text + vector line
    def draw_vector_field(label, line_idx):
        y = get_y(line_idx)
        c.drawString(info_x, y, label)
        # Draw solid vector line at baseline (or slightly below? typically baseline)
        c.saveState()
        c.setLineWidth(0.5)
        c.line(field_value_x, y, field_value_x + field_line_length, y)
        c.restoreState()
    ...
    # v20 REFLOW PAGE 2 BODY (Prevent Overflow)
    # Line 14
    ...
    # Line 16 (Reflowed)
    c.drawString(MARGIN_LEFT, get_y(16), "(\"The First Action\") challenges the validity of the lease and seeks rescission.") # Manual wrap check: approx 70 chars. Safe.
    
    # Line 17 (Reflowed)
    c.drawString(MARGIN_LEFT, get_y(17), "The determination of the lease's validity in the First Action is a prerequisite")
    
    # Line 18 (Reflowed)
    c.drawString(MARGIN_LEFT, get_y(18), "to any adjudication of possession in this summary proceeding. A \"Notice of")
    
    # Line 19 (Reflowed)
    c.drawString(MARGIN_LEFT, get_y(19), "Related Case\" linking these matters was filed on February 3, 2026.")
    ...
    # v20 VECTOR SIGNATURE LINE (Page 3 Top)
    # Line 2: Signature Line (Vector)
    c.saveState()
    c.setLineWidth(1.0) # Signature lines usually thicker? Or standard 0.5? Let's go 1.0 or standard. Standard is usually fine, matches form fields.
    # Prompt says "Length: 35 chars approx 2.5 inches".
    ...
    # v19 TITANIUM LOCK: EXACT GRID SNAP (From v18 decision)
    ...
    # v20 VECTOR SIGNATURE LINE (Page 3 Bottom)
    sig_y = get_y(26)
    c.saveState()
    c.setLineWidth(1.0)
    c.line(MARGIN_LEFT, sig_y, MARGIN_LEFT + sig_line_len, sig_y)
    c.restoreState()
    # c.drawString(MARGIN_LEFT, sig_y, "_" * 35) # REMOVED
    
    # Line 27: Name (GRID SNAP - EXACT)
    c.setFont("Times-Bold", 12)
    c.drawString(MARGIN_LEFT, get_y(27), "NUHA SAYEGH") 
    
    # Line 28: Title (GRID SNAP - EXACT)
    c.setFont("Times-Roman", 12)
    c.drawString(MARGIN_LEFT, get_y(28), "Defendant in Pro Per") 
    
    c.save()
    print(f"Final Filing Ready PDF generated at: {FULL_OUTPUT_PATH}")
