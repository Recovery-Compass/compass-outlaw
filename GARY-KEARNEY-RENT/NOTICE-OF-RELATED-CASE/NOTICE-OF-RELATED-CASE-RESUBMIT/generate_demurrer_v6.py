...
# --- Configuration ---
OUTPUT_FILENAME = "DEMURRER_FINAL_FILED_READY.pdf"
...
MARGIN_TOP = 0.9 * inch  # Raised Grid Start (v5 Fix Retained)
MARGIN_BOTTOM = 0.35 * inch # Lowered Footer (v5 Fix Retained)
...
def create_demurrer_v6():
    ...
    # Vertical Separator: Parentheses Stack
    sep_x = 4.5 * inch
    c.setFont("Times-Roman", 12)
    for l_num in range(11, 21):
        c.drawString(sep_x, get_y(l_num), ")")
    ...
    c.setFont("Times-Roman", 12)
    c.drawString(info_x, get_y(17), "Date: _________________")
    c.drawString(info_x, get_y(18), "Time: _________________")
    c.drawString(info_x, get_y(19), "Dept: _________________")
    ...
    # Page 2 ends here (Line 26). Line 27-28 Blank.
    # Date moved to Page 3.
    ...
    sig_line_y = get_y(2)
    c.drawString(MARGIN_LEFT, sig_line_y, "_" * 35)
    
    # Line 2.5/3: Name and Title
    # Draw slightly below Line 2 baseline.
    c.setFont("Times-Bold", 12)
    c.drawString(MARGIN_LEFT, sig_line_y - 12, "NUHA SAYEGH")
    c.setFont("Times-Roman", 12)
    c.drawString(MARGIN_LEFT, sig_line_y - 24, "Defendant in Pro Per")
    
    # Line 6: Memorandum Header (Moved down from Line 5 usually)
    c.setFont("Times-Bold", 12)
    c.drawCentredString(PAGE_WIDTH/2, get_y(6), "MEMORANDUM OF POINTS AND AUTHORITIES")
    
    # Intro
    c.drawString(MARGIN_LEFT, get_y(8), "I. INTRODUCTION")
    c.setFont("Times-Roman", 12)
    c.drawString(MARGIN_LEFT, get_y(9), "This Unlawful Detainer action is a retaliatory attempt to enforce a void lease on an")
    c.drawString(MARGIN_LEFT, get_y(10), "illegal dwelling. Plaintiff filed this action on January 28, 2026. However, Defendant")
    c.drawString(MARGIN_LEFT, get_y(11), "had already filed a unlimited civil action against Plaintiff on January 21, 2026")
    
    # Line 12 (Previously 11)
    draw_manual_rich_line(c, MARGIN_LEFT, 12, [
        ("(", "Times-Roman"),
        ("Sayegh v. Kearney", "Times-Italic"),
        (", Case No. 26NNCV00412), alleging Fraud and seeking", "Times-Roman")
    ])
    
    c.drawString(MARGIN_LEFT, get_y(13), "Rescission. Because the First Action challenges the existence of the landlord-tenant")
    c.drawString(MARGIN_LEFT, get_y(14), "relationship and was filed first, this Court should sustain the demurrer.")
    
    c.setFont("Times-Bold", 12)
    c.drawString(MARGIN_LEFT, get_y(16), "II. THE LEASE IS VOID AB INITIO (CCP § 430.10(e))")
    c.setFont("Times-Roman", 12)
    c.drawString(MARGIN_LEFT, get_y(17), "A contract for an illegal purpose is void (Civil Code § 1598). A lease for a unit that")
    ...
    # Line 19
    draw_manual_rich_line(c, MARGIN_LEFT, 19, [
        ("Calva", "Times-Italic"),
        ("). Here, the premises violate Temple City Municipal Code regarding density", "Times-Roman")
    ])
    
    c.drawString(MARGIN_LEFT, get_y(20), "and mandatory access width. A 3-Day Notice that demands rent for an illegal unit")
    c.drawString(MARGIN_LEFT, get_y(21), "is invalid on its face.")
    
    c.setFont("Times-Bold", 12)
    c.drawString(MARGIN_LEFT, get_y(23), "III. FIRST-IN-TIME PRIORITY (CCP § 430.10(c))")
    c.setFont("Times-Roman", 12)
    c.drawString(MARGIN_LEFT, get_y(24), "Under CCP § 430.10(c), a party may demur when \"there is another action pending")
    c.drawString(MARGIN_LEFT, get_y(25), "between the same parties on the same cause of action.\" The validity of the Lease")
    
    # Line 26
    draw_manual_rich_line(c, MARGIN_LEFT, 26, [
    ...
    # Wait, prompt says "Keep Line 28 Signature" in previous? 
    # v6 PROMPT SAYS: "Move entire Execution Block to Page 3 Top."
    # So Line 28 on Page 3 is NOW "DATED: February 3, 2026" again? 
    # No, that would duplicate.
    # The Prompt says: "Move the Entire Execution Block (Date + Signature + Name) to the Top of Page 3."
    # ... "Line 3 (Offset): Draw 'Defendant in Pro Per' below the name." 
    #
    # AND "Shift Memorandum Header down to Line 5 or 6".
    #
    # Wait, is there a Second Signature at end of Memorandum?
    # Original content had "DATED: ..." and Signature on Page 2 (Demurrer)
    # AND "DATED: ..." and Signature on Page 3 (Memory of Points and Authorities).
    #
    # Page 1: Notice.
    # Page 2: Demurrer Grounds + Prayer + Signature (Demurrer).
    # Page 3: Points and Authorities + Signature (Memo).
    #
    # The "Widowed Signature" error refers to Page 2 having Date but Signature falling to Page 3 Line 1?
    #
    # Re-reading Prompt v6:
    # "Fix the 'Widowed Signature' error where the Date appears on Page 2 and the Signature on Page 3. Both must be on the same page."
    # "Move the entire Execution Block to the Top of Page 3."
    #
    # So for the DEMURRER itself, we sign on Page 3 Top.
    # AND then the MEMORANDUM starts.
    # Does the Memorandum need a signature?
    # Standard: Yes, usually at end.
    #
    # The script currently has a signature at Page 3 Line 28 as well (Line 27 Dated, Line 28 Signature Line).
    #
    # So Page 3 Top: Signature for Demurrer.
    # Page 3 Bottom: Signature for Memorandum.
    #
    # My code above:
    # Page 3 Top: Execution Block.
    # Line 28: "DATED: February 3, 2026".
    # I did NOT put a signature line at bottom of Page 3 in the code above yet. I just put Date.
    # The v6 Prompt says: "RETAIN PREVIOUS FIXES".
    # v4/v5 had a signature block on Page 3 Line 28 (Name under line).
    #
    # I should add the Signature Line to Page 3 Bottom as well if it's required for the Memorandum.
    # Usually a Demurrer has one signature for the whole Pleading + Memo, or two.
    # The Prompt v6 mainly talks about the "Widowed Signature" (the one for the Demurrer prayer).
    #
    # I will add the Signature Line at bottom of Page 3 as well to be safe (Memo signature), 
    # mimicking formatting of others.
    # Or just follow what was there: "DATED: ..." and "_" * 35.
    
    # Line 28: Date.
    # I will draw signature line below Line 28 or at Line 29 (overflow?)
    # Layout of Page 3 Bottom:
    # Line 26: Quote end.
    # Line 28: Date.
    # Where does signature go?
    # v5 had "Signature Page 3 - CRITICAL FIX ... Line 28".
    #
    # I will replicate the "Signature Unity" at the top of Page 3 (for the Demurrer).
    # And at the bottom of Page 3 (for the Memo), I will place "DATED: ..." on Line 27 and Signature on Line 28.
    # The code above at Line 26 is end of text.
    # Line 27: DATED.
    # Line 28: Signature.
    # This fits.
    ...
    # Signature line Line 28
    sig_y_bottom = get_y(28)
    c.drawString(MARGIN_LEFT, sig_y_bottom, "_" * 35)
    c.setFont("Times-Bold", 12)
    c.drawString(MARGIN_LEFT, sig_y_bottom - 12, "NUHA SAYEGH")
    c.setFont("Times-Roman", 12)
    c.drawString(MARGIN_LEFT, sig_y_bottom - 24, "Defendant in Pro Per")
    
    c.save()
    print(f"Final Filed Ready PDF generated at: {FULL_OUTPUT_PATH}")
