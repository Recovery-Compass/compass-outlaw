...
# --- Configuration ---
OUTPUT_FILENAME = "DEMURRER_FINAL_GOLD_MASTER.pdf"
...
def create_demurrer_v9():
    ...
    # Line 4: Memorandum Header (Moved UP from Line 5)
    c.setFont("Times-Bold", 12)
    c.drawCentredString(PAGE_WIDTH/2, get_y(4), "MEMORANDUM OF POINTS AND AUTHORITIES")
    
    # Shift ALL subsequent content up by 1 line relative to v8 (2 lines relative to v7)
    # v8 had Intro at 7. v9 has it at 6.
    c.drawString(MARGIN_LEFT, get_y(6), "I. INTRODUCTION")
    c.setFont("Times-Roman", 12)
    c.drawString(MARGIN_LEFT, get_y(7), "This Unlawful Detainer action is a retaliatory attempt to enforce a void lease on an")
    c.drawString(MARGIN_LEFT, get_y(8), "illegal dwelling. Plaintiff filed this action on January 28, 2026. However, Defendant")
    c.drawString(MARGIN_LEFT, get_y(9), "had already filed a unlimited civil action against Plaintiff on January 21, 2026")
    
    # Line 10 (was 11)
    draw_manual_rich_line(c, MARGIN_LEFT, 10, [
        ("(", "Times-Roman"),
        ("Sayegh v. Kearney", "Times-Italic"),
        (", Case No. 26NNCV00412), alleging Fraud and seeking", "Times-Roman")
    ])
    
    c.drawString(MARGIN_LEFT, get_y(11), "Rescission. Because the First Action challenges the existence of the landlord-tenant")
    c.drawString(MARGIN_LEFT, get_y(12), "relationship and was filed first, this Court should sustain the demurrer.")
    
    c.setFont("Times-Bold", 12)
    c.drawString(MARGIN_LEFT, get_y(14), "II. THE LEASE IS VOID AB INITIO (CCP § 430.10(e))")
    c.setFont("Times-Roman", 12)
    c.drawString(MARGIN_LEFT, get_y(15), "A contract for an illegal purpose is void (Civil Code § 1598). A lease for a unit that")
    ...
    c.drawString(MARGIN_LEFT, get_y(18), "and mandatory access width. A 3-Day Notice that demands rent for an illegal unit")
    c.drawString(MARGIN_LEFT, get_y(19), "is invalid on its face.")
    
    c.setFont("Times-Bold", 12)
    c.drawString(MARGIN_LEFT, get_y(21), "III. FIRST-IN-TIME PRIORITY (CCP § 430.10(c))")
    c.setFont("Times-Roman", 12)
    c.drawString(MARGIN_LEFT, get_y(22), "Under CCP § 430.10(c), a party may demur when \"there is another action pending")
    c.drawString(MARGIN_LEFT, get_y(23), "between the same parties on the same cause of action.\" The validity of the Lease")
    
    # Line 24 (was 25)
    draw_manual_rich_line(c, MARGIN_LEFT, 24, [
        ("is the subject of the prior pending unlimited civil action (", "Times-Roman"),
        ("Case No. 26NNCV00412", "Times-Italic"),
        (").", "Times-Roman")
    ])
    
    # New Bottom Date: Line 25
    c.drawString(MARGIN_LEFT, get_y(25), "DATED: February 3, 2026")
    
    # New Bottom Signature: Line 26
    sig_y_bottom = get_y(26)
    ...
    # Line 27-28 is now empty.
    
    c.save()
    print(f"Final Gold Master PDF generated at: {FULL_OUTPUT_PATH}")
